...
> **Note:** `NetworkTopology` collects its own Virtual Network, peering, Gateway, Firewall, and Network Virtual Appliance data independently of the `VirtualNetwork`, `VirtualNetworkGateway`, `Firewall`, and `NetworkVirtualAppliance` `InfoLevel` settings — only `NetworkTopology` itself controls whether the diagram is collected and rendered, so a hub is never misclassified as a spoke, or the diagram omitted entirely, just because another section is disabled or set to a lower level elsewhere in the report.

### Diagram
The **Diagram** schema allows each generated diagram to be independently enabled and configured. Requires the [AsBuiltReport.Diagram](https://github.com/AsBuiltReport/AsBuiltReport.Diagram) module.

| Sub-Schema     | Setting       | Default | Description                                                                                                                                              |
|----------------|---------------|---------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ManagementGroup.Enabled  | true / false  | false   | Toggle to enable/disable the Management Group hierarchy diagram.                                                                                          |
| ManagementGroup.Theme    | White / Black | White   | Sets the colour theme for the Management Group diagram.                                                                                                   |
| ManagementGroup.Dpi      | 72 - 600      | 96      | Sets the raster output resolution (dots per inch) for the Management Group diagram. Requires [AsBuiltReport.Diagram](https://github.com/AsBuiltReport/AsBuiltReport.Diagram) 1.0.8 or later. |
